...
def transition_smoothing():
    ...
    for tag in tag_list:
        if "start" + tag not in transition_prob:
            transition_prob[("start" + "~tag~" + tag)] = Decimal(1) / \
                Decimal(len(word_set) + tag_count[tag])
    for tag1 in tag_list:
        for tag2 in tag_list:
            if (tag1 + "~tag~" + tag2) not in transition_prob:
                transition_prob[(tag1+"~tag~"+tag2)] = Decimal(1) / \
                    Decimal(len(word_set) + tag_count[tag1])
...
def emission_probability():
    ...
    for key in word_count:
        emission_prob_dict[key] = Decimal(
            word_count[key])/tag_count[key.split("/")[-1]]
